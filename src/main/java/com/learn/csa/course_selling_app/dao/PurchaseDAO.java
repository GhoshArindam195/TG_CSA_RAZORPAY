...
public class PurchaseDAO {
    ...
    public static boolean savePurchase(String orderId) {
        ...
        try {
            ...
            sql = "UPDATE PURCHASES SET COURSE_STARTING = (SELECT C_STARTING FROM COURSES WHERE COURSES.C_ID = PURCHASES.COURSE_ID) WHERE PAYMENT_ID IN (SELECT payment_id FROM ORDERS WHERE O_ID = ?)";
    ...
    {
        List<Purchase> purchases=new ArrayList<>();//blank arraylist is created to return every course
        try{
            Connection con= Connection_Provider.getConnection();
            ...
            while(set.next())
            {
                Purchase purchase=new Purchase(set.getInt(1),set.getString(2),set.getString(3),set.getString(4), set.getInt(5), set.getInt(6),set.getString(7),set.getString(8));
        ...
        return purchases;//if no course and get into the database then size will be returned as 0
    }
